...
A decentralized lottery system (Raffle) built with Solidity and Foundry. This project demonstrates how to build an automated, verifiable, and tamper-proof smart contract application.
...
checkUpkeep(): Checks if the conditions for a draw are met (Is enough time passed? Is there ETH? Are there players?).
...
📝 Project Learnings
Foundry Scripting: Learned how to write complex deployment and management scripts using Solidity, replacing traditional JS/TS scripts.

Chainlink Integration: Deep dive into the VRF Subscription Manager model, implementing a fully automated Create -> Fund -> AddConsumer workflow via code.

Testing: Mastered vm.warp (time manipulation) and vm.roll (block manipulation) to test time-dependent smart contract logic.
